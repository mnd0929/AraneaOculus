...
{
    internal class NetworkLoadChartController
    {
        public static void Display(Chart chart, ObservableDictionary<Guid, ObservableCollection<PacketHistoryEntry>> entries)
        {
            Debug.WriteLine("[NetworkLoadChartController] START");
            ...
            var allEntries = entries.Values.SelectMany(list => list).OrderBy(e => e.Timestamp).ToList();
            if (allEntries.Count < 2)
            {
                ...
                Debug.WriteLine("[NetworkLoadChartController] STOP");
                return;
            }
            ...
            {
                var ordered = deviceHistory.OrderBy(e => e.Timestamp).ToList();
